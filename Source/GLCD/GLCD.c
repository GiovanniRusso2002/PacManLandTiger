...
void LCD_StandardPill( uint16_t x0, uint16_t y0, uint16_t color ){
	LCD_DrawLine(x0*8+3+8, y0*8+3+40, x0*8+4+8, y0*8+3+40, color);
	LCD_DrawLine(x0*8+3+8, y0*8+4+40, x0*8+4+8, y0*8+4+40, color);
}

void LCD_PowerPill( uint16_t x0, uint16_t y0, uint16_t color ){
	int i=0;
	for(i=0;i<4;i++){
		LCD_DrawLine(x0*8+2+8, y0*8+2+40+i, x0*8+5+8, y0*8+2+40+i, color);
	}
...
void LCD_DrawSquare( uint16_t x0, uint16_t y0, uint16_t color ){
	int i;
	for(i=0;i<8;i++){
		LCD_DrawLine(x0*8+8, y0*8+i+40, x0*8+8+8, y0*8+i+40, color);
	}
}
...
void LCD_DeleteStaff( uint16_t x0, uint16_t y0, uint16_t color ){
	int i;
	for(i=0;i<7;i++){
		LCD_DrawLine(x0*8+8+1, y0*8+i+40+1, x0*8+8+7, y0*8+i+40+1, color);
	}
}


void LCD_DrawPacman( uint16_t x0, uint16_t y0, uint16_t color ){
	// Righe da disegnare: ogni elemento dell'array rappresenta l'inizio e la fine delle righe orizzontali
    // Ogni elemento {start, end} indica l'intervallo di pixel accesi per quella riga
    uint8_t pacmanLines[5][2] = {
        {3, 5},
        {2, 6},
        {2, 6},
        {2, 6},
    ...
    // Disegna ogni riga orizzontale usando LCD_DrawLine
		int y;
    for (y = 0; y < 5; y++) {
        uint8_t start = pacmanLines[y][0]; // Colonna iniziale della riga
        uint8_t end = pacmanLines[y][1];   // Colonna finale della riga
        LCD_DrawLine((1+x0)*8 + start, (5+y0)*8 + y + 1, (1+x0)*8 + end, (5+y0)*8 + y + 1, color); // Disegna la riga orizzontale
    }	
}
